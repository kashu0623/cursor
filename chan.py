...
if __name__ == "__main__":
    ...
    print("🎯 기본 실행: 새로운 듀얼 입력 모델 (Raw + 5개 HR/HRV Features)")
    print("📝 참고용: 기존 단일 입력 모델은 주석 처리되어 있음 (필요시 주석 해제)")
    
    # === 기본 실행: 새로운 듀얼 입력 모델 (Raw + 5개 HR/HRV Features) ===
    print("\n" + "="*60)
    print("🚀 새로운 듀얼 입력 모델 학습 시작")
    print("   - 원시 신호 (BVP, 가속도) + 5개 HR/HRV 피처")
    print("   - LSTM + MLP 결합 아키텍처")
    print("   - 로그 스케일 클래스 가중치 적용")
    print("   - 출력 파일: dreamt_pretrained_dual_input_5features_log_weights.pth")
    print("="*60)
    ...
    try:
        ...
        print("\n🎉 듀얼 입력 모델 학습 성공적으로 완료!")
        print("   저장된 모델: dreamt_pretrained_dual_input_5features_log_weights.pth")
    except Exception as e:
        print(f"\n❌ 듀얼 입력 모델 학습 실패: {e}")
        print("   오류가 발생했지만 프로그램은 계속 실행됩니다.")
    
    # === 참고용: 기존 단일 입력 모델 (Raw LSTM) - 주석 처리됨 ===
    # print("\n" + "="*60)
    # print("📚 기존 단일 입력 모델 학습 (참고용)")
    # print("   - 원시 신호만 사용하는 LSTM 모델")
    # print("   - 출력 파일: dreamt_pretrained_legacy.pth")
    # print("="*60)
    # 
    # try:
    #     pretrain_on_dreamt(
    #         data_dir=r"C:\dreamt_pretrain",
    #         output_path="dreamt_pretrained_legacy.pth"
    #     )
    #     print("\n✅ 기존 모델 학습 완료!")
    #     print("   저장된 모델: dreamt_pretrained_legacy.pth")
    # except Exception as e:
    #     print(f"\n❌ 기존 모델 학습 실패: {e}")
    #     print("   오류가 발생했지만 프로그램은 계속 실행됩니다.")
    ...
    # === 향후 8채널 확장 예시 (주석 처리) ===
    # print("\n" + "="*60)
    # print("🔮 향후 8채널 확장 예시 (참고용)")
    # print("   - IR, Red, 가속도, 자이로스코프 등 8채널 지원")
    # print("="*60)
    # FUTURE_SIGNAL_CHANNELS = ['ir', 'red', 'acc_x', 'acc_y', 'acc_z', 'gyro_x', 'gyro_y', 'gyro_z']
    # pretrain_on_dreamt_dual_input(
    #     data_dir=r"C:\Users\ahrid\dreamt_pretrain",
    #     output_path="dreamt_pretrained_dual_input_8ch.pth",
    #     signal_channels=FUTURE_SIGNAL_CHANNELS,
    #     label_column='label'
    # )
    
    # === Fine-tuning 예시 (주석 처리) ===
    # print("\n" + "="*60)
    # print("🎯 Fine-tuning 예시 (참고용)")
    # print("   - 실제 데이터로 모델 미세 조정")
    # print("="*60)
    # finetune_on_actual(
    ...
    print("\n" + "="*60)
    print("🏁 프로그램 실행 완료!")
    print("   - 듀얼 입력 모델이 기본으로 학습되었습니다.")
    print("   - 기존 모델이 필요하시면 주석을 해제하세요.")
    print("="*60)
